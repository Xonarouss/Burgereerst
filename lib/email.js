...
export async function sendBlogNewPostEmail({ to, locale, title, excerpt, url, unsubscribeUrl }) {
  ...
  const unsubLabel = locale === "en" ? "Unsubscribe" : "Afmelden";
  const footerHint =
    locale === "en"
      ? "You are receiving this because you subscribed to BurgerEerst blog updates."
      : "Je ontvangt dit omdat je je hebt geabonneerd op blog-updates van BurgerEerst.";
    ...
    resend.emails.send({
      ...
      html: `
        <div style="font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial; line-height: 1.5; color: #0f172a;">
          ...
          <p style="margin:0 0 6px 0; color:#64748b; font-size:12px;">${footerHint}</p>
          ${unsubscribeUrl ? `<p style="margin:0; color:#64748b; font-size:12px;"><a href="${unsubscribeUrl}" style="color:#64748b; text-decoration:underline;">${unsubLabel}</a></p>` : ""}
        </div>
      `,
    }),
    timeoutMs
